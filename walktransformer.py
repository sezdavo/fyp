...
class Classifier(nn.Module):
    def __init__(self, embed_size):
        ...
        self.hid1 = nn.Linear(embed_size, 8)  # 4-(8-8)-1
...
# BUILD AND CREATE INPUT TO OBJECT EMBEDDING MLPs
class pos2vec(nn.Module):
    def __init__(self, out_size):
        super(pos2vec, self).__init__()
        self.hid1 = nn.Linear(2, 8) # 2-(8)-16
        self.drop1 = nn.Dropout(0.50)
        self.oupt = nn.Linear(8, out_size)
    ...
    def forward(self, pos):
        z = torch.tanh(self.hid1(pos)) 
        z = self.drop1(z)
        z = torch.sigmoid(self.oupt(z))
        return z

class box2vec(nn.Module):
    def __init__(self, out_size):
        super(box2vec, self).__init__()
        self.hid = nn.Linear(1, 8) # 1-(8)-16
    ...
    def forward(self, box):
        z = torch.tanh(self.hid(box)) 
        z = self.drop(z)
        z = torch.sigmoid(self.oupt(z))
        return z

class speed2vec(nn.Module):
    def __init__(self, out_size):
        super(speed2vec, self).__init__()
        self.hid = nn.Linear(1, 8) # 1-(8)-16
        self.drop = nn.Dropout(0.50)
        self.oupt = nn.Linear(8, out_size)
...
class Trunk(nn.Module):
    def __init__(
        ...
        self.time_embedding = nn.Embedding(30, out_size)
        self.pos2vec = pos2vec(out_size)
        self.box2vec = box2vec(out_size)
        self.speed2vec = speed2vec(out_size)
    ...
    def forward(self, pos, box, speed):
        # x is the vector containing the object information
        # Grab info about x for grabbing time embeddings
        # N = Batch Size, seq_length = number of objects in sequence (30)
        N = 32
        seq_length = 30
        # Create position vector ([1,2...30] x N )
        positions = torch.arange(0, seq_length).expand(N, seq_length).to(self.device)
        time_embedding = self.time_embedding(positions).to(self.device)
        # Get other embedding vectors
        spatial_embedding = self.pos2vec(pos).to(self.device)
        depth_embedding = self.box2vec(box).to(self.device)
        speed_embedding = self.speed2vec(speed).to(self.device)
        # Concatenate all embedding vectors (1x16) into a final embedding vector (1x64)
        x = torch.cat((time_embedding, spatial_embedding, depth_embedding, speed_embedding), 2).to(self.device)
        out = self.dropout(x)
        # Get query from final sequence of embedding vectors
        N, seq_length, embed_size = x.shape
        q = torch.index_select(x, 1, torch.tensor([29]).to(self.device)).to(self.device)
        q = self.dropout(q)
